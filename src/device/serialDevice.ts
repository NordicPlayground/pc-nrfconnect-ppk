...
import { bulkInsertDB, insertDB } from '../features/dbConnection';
...
class SerialDevice extends Device {
    ...
    sendCommand(cmd: PPKCmd) {
        ...
        if (cmd[0] === PPKCmd.AverageStart) {
            ...
            this.consecutiveRangeSample = 0;
            this.afterSpike = 0;
            this.sendCommandDescription(PPKCmd.AverageStart);
        }
        this.child.send({ write: cmd });
        return Promise.resolve(cmd.length);
    }

    sendCommandDescription(type: string | number, value?: any) {
        this.child.send({ commandType: type, value });
    }
    ...
    ppkSetSpikeFilter(spikeFilter: SpikeFilter): void {
        ...
        this.sendCommandDescription('SET_SPIKE_FILTER', this.spikeFilter);
    }
